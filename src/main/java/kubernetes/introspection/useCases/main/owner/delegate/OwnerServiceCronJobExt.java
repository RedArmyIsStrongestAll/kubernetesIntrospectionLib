...
public class OwnerServiceCronJobExt extends OwnerService {
    ...
    public OwnerServiceCronJobExt(KubernetesOwnerPort ownerPort, String namespace) {
        super(ownerPort, namespace);
    }

    @Override
    protected String getNameClassExt() { return SERVICE_NAME; }

    @Override
    protected OwnerTypeEnum getKindOwnerType() { return OWNER_TYPE; }
    ...
    public OwnerDto getOwnerDto(OwnerReferenceInfo ownerRef) {
        log.info("{}: fetching CronJob: {}", SERVICE_NAME, ownerRef.getName());
        OwnerInfo ownerInfo = ownerPort.getCronJob(ownerRef.getName(), namespace);
