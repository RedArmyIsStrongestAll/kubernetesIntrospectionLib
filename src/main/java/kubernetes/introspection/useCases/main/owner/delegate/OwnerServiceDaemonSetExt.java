...
public class OwnerServiceDaemonSetExt extends OwnerService {
    ...
    public OwnerServiceDaemonSetExt(KubernetesOwnerPort ownerPort, String namespace) {
    ...
    public OwnerDto getOwnerDto(OwnerReferenceInfo ownerRef) {
        log.info("{}: fetching DaemonSet: {}", SERVICE_NAME, ownerRef.getName());
        OwnerInfo ownerInfo = ownerPort.getDaemonSet(ownerRef.getName(), namespace);
